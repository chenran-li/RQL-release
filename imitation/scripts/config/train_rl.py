...
def train_rl_defaults():
    ...
    normalize_reward = False  # Use VecNormalize to normalize the reward
...
# Residual Q-learning experiment environments
@train_rl_ex.named_config
def parking_basic():
    environment = dict(gym_id="parking-basic-v0", num_vec=4)
    total_timesteps = int(8e5)
    policy = dict(policy_kwargs=dict(net_arch=[256, 256, 256]))


@train_rl_ex.named_config
def parking_total():
    environment = dict(gym_id="parking-basic-boundaryall-v0", num_vec=4)
    total_timesteps = int(8e5)
    policy = dict(policy_kwargs=dict(net_arch=[256, 256, 256]))


@train_rl_ex.named_config
def mountain_car_basic():
    environment = dict(gym_id="MountainCarContinuous-modifed-v0", num_vec=1)
    total_timesteps = int(1e5)
    policy = dict(policy_kwargs=dict(log_std_init=-3.67, net_arch=[64, 64]))


@train_rl_ex.named_config
def mountain_car_total():
    environment = dict(gym_id="MountainCarContinuous-modifed-lessleftall-v0", num_vec=1)
    total_timesteps = int(1e5)
    policy = dict(policy_kwargs=dict(log_std_init=-3.67, net_arch=[64, 64]))
...
@train_rl_ex.named_config
def cartpole_basic():
    environment = dict(gym_id="CartPole-modifed-v1", num_vec=1)
    total_timesteps = int(1e5)
...
@train_rl_ex.named_config
def cartpole_total():
    environment = dict(gym_id="CartPole-modifed-morecenterall-v1", num_vec=1)
    total_timesteps = int(1e5)
    policy = dict(policy_kwargs=dict(net_arch=[256, 256]))
...
@train_rl_ex.named_config
def highway_basic():
    environment = dict(gym_id="highway-ME-basic-v0", num_vec=1)
...
@train_rl_ex.named_config
def highway_total():
    environment = dict(gym_id="highway-ME-basic-AddRightRewardALL-v0", num_vec=1)
    total_timesteps = int(7e5)
    policy = dict(policy_kwargs=dict(net_arch=[256, 256]))
