...
inverted_ALGOS = {v: k for k, v in ALGOS.items()}
...
def config():
    ...
    batch_size = None
    rl_kwargs = dict()
    hyperparameter_directory = str(pathlib.Path(__file__).parent.parent.parent.parent / "hyperparams")
...
def config_hook(config, command_name, logger):
    ...
    if config["rl"]["rl_cls"] is None:
        config["rl"]["rl_cls"] = sb3.PPO
    res["rl_cls"] = config["rl"]["rl_cls"]
    algorithm = inverted_ALGOS[res["rl_cls"]]

    # read configuration from stablebaseline3 hyperparameter files
    hyperparameter_file = config["rl"]["hyperparameter_directory"] + f"/{algorithm}.yml"
    with open(hyperparameter_file) as f:
        hyperparams_dict = yaml.safe_load(f)
    if config["environment"]["gym_id"] in list(hyperparams_dict.keys()):
        hyperparams = hyperparams_dict[config["environment"]["gym_id"]]
    else:
        raise ValueError(f"Hyperparameters for {config['environment']['gym_id']} not found in {hyperparameter_file}")
    
    # remove redundant hyperparameters
    n_envs = config["environment"]["num_vec"]
    ...
    # set rl_kwargs and batch_size
    if "batch_size" not in hyperparams.keys():
        hyperparams["batch_size"] = 256
    res["rl_kwargs"] = hyperparams
    if issubclass(res["rl_cls"], on_policy_algorithm.OnPolicyAlgorithm):
        res["batch_size"] = hyperparams["n_steps"] * n_envs
        del res["rl_kwargs"]["n_steps"]
    if issubclass(res["rl_cls"], off_policy_algorithm.OffPolicyAlgorithm):
        res["batch_size"] = hyperparams["batch_size"]
        res["rl_kwargs"]["batch_size"] = None 

    # set use_sde to False for imitation
    if 'demonstrations' in config.keys():
        res["rl_kwargs"]["use_sde"] = False
...
def dqn_ME():
    rl_cls = sb3.DQN_ME
    warnings.warn(
        "Soft DQN currently only supports discrete action spaces. ",
        category=RuntimeWarning,
    )
    locals()  # quieten flake8
...
def make_rl_algo(
    ...
    if issubclass(rl_cls, on_policy_algorithm.OnPolicyAlgorithm):
        ...
        if rl_kwargs.get("batch_size") is None:
