...
class KinematicsGoalObservationVectorlize(KinematicObservation):
    def __init__(self, env: 'AbstractEnv', scales: List[float], **kwargs: dict) -> None:
        self.scales = np.array(scales)
        super().__init__(env, **kwargs)

    def space(self) -> spaces.Space:
        return spaces.Box(-np.inf, np.inf, shape=(2, len(self.features)), dtype=np.float64)

    def observe(self) -> np.ndarray:
        if not self.observer_vehicle:
            return np.zeros((2,len(self.features)))
        obs = np.ravel(pd.DataFrame.from_records([self.observer_vehicle.to_dict()])[self.features]) / self.scales
        goal = np.ravel(pd.DataFrame.from_records([self.env.goal.to_dict()])[self.features]) / self.scales

        new_obs = np.concatenate((goal.reshape(1,len(self.features)), obs.reshape(1,len(self.features))),axis = 0)
        return new_obs
...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    ...
    elif config["type"] == "KinematicsGoalVectorlized":
        return KinematicsGoalObservationVectorlize(env, **config)
